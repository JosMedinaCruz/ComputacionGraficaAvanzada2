...
// Modelo para el render del texo
...
// Blending model unserted (vector con los objetos )
...
void init(int width, int height, std::string strTitle, bool bFullScreen) {
	...
	// Se inicializa el modelo de texeles para dibujar texto
...
void applicationLoop() {
	...
	while (psi) {
		...
		glDisable(GL_CULL_FACE); //Para desactivarlo 
		modelGrass.render();
		glEnable(GL_CULL_FACE);  //Para poder ver bien en todas direcciones el grass
		...
		// Actualizar la posición con objetos con mezcla de color ---> Practica 09
		// Update the aircraft
		blendingUnsorted.find("aircraft")->second = glm::vec3(modelMatrixAircraft[3]); // Para encontrar por la etiqueta el objeto
		// Actualizar posición de lambo
		blendingUnsorted.find("lambo")->second = glm::vec3(modelMatrixLambo[3]);
		// Actualiza posición del helicoptero
		blendingUnsorted.find("heli")->second = glm::vec3(modelMatrixHeli[3]); //Con el 3 se accede a la posición de esa matriz de transformación
		...
		for (itBlend = blendingUnsorted.begin(); itBlend != blendingUnsorted.end(); itBlend++) {
			...
			blendingOrdenado[distanceFromView] = std::make_pair(itBlend->first, itBlend->second); //llave es el nombre, valor es la posición
		}

		// Render de las transparencias
		glEnable(GL_BLEND);
		//glBlendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA);// ahora con el for se itera de forma inversa
		for (std::map<float, std::pair<std::string, glm::vec3>>::reverse_iterator it = blendingOrdenado.rbegin(); it != blendingOrdenado.rend(); it++) {
			...
			if (it->second.first.compare("aircraft") == 0) {
				...
				glBlendFunc(GL_SRC_ALPHA, GL_ONE); //Hace transparente todo el carro
				// Lambo car
				glDisable(GL_CULL_FACE);
		...
		/*******************************************
		 * Render de colliders                           /////   Se comenta para ya no verlas hitboxes
		 *******************************************/
		...
		modelText->render("Un contador con text rendering", -0.5, -0.9, 30, 0.5, 0.3, 0.6); // x y size r g b   Checar por qué no cambia de tamaño y color
		glDisable(GL_BLEND);
