...
void initParticlesBuffers() {
	...
	for (unsigned int i = 0; i < nParticles; i++) {
		theta = glm::mix<float>(0.0, glm::pi<float>() , ((float)rand() / RAND_MAX)); //theta = glm::mix<float>(0.0, glm::pi<float>() / 6.0, ((float)rand() / RAND_MAX));
		phi = glm::mix<float>(0.0, glm::two_pi<float>(), ((float)rand() / RAND_MAX));
		v.x = sinf(theta) * cosf(phi);
		v.y = cosf(theta);
		v.z = sinf(theta) * sinf(phi);
		velocity = glm::mix<float>(0.1, 0.9, ((float)rand() / RAND_MAX)); //velocity = glm::mix<float>(0.6, 0.8, ((float)rand() / RAND_MAX));
	...
	while (psi) {
		...
		for (std::map<float, std::pair<std::string, glm::vec3> >::reverse_iterator it = blendingSorted.rbegin(); it != blendingSorted.rend(); it++) {
			if (it->second.first.compare("aircraft") == 0) {
				...
				shaderFountainParticles.setVectorFloat3("Gravity", glm::value_ptr(glm::vec3(0.0f, -0.4, 0.0f))); //-0.1 es para la gravedad
