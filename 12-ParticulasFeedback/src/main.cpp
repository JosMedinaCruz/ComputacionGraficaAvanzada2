...
GLuint nParticlesFire = 4000;
GLuint posBuf[2], velBuf[2], age[2];
GLuint particleArray[2];
GLuint feedback[2];
GLuint drawBuf = 1; 
float particleSize = 0.4, particleLifetime = 2.0;
...
void init(int width, int height, std::string strTitle, bool bFullScreen) {
	...
	shaderParticlesFire.initialize("../Shaders/particlesLluvia.vs", "../Shaders/particlesLluvia.fs", {"Position", "Velocity", "Age"});  //Para cambiar como fuego o Smoke
	...
	while (psi) {
		...
		for(std::map<float, std::pair<std::string, glm::vec3> >::reverse_iterator it = blendingSorted.rbegin(); it != blendingSorted.rend(); it++){
			if(it->second.first.compare("aircraft") == 0){
				...
				shaderParticlesFire.setVectorFloat3("colorHumo", glm::value_ptr(glm::vec3(1.0, 1.0, 1.0)));
